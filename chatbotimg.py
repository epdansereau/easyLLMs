...
from tools import multiply, add, current_time, random_number, generate_image, generate_video
...
def gradio_history_to_langchain_history(gradio_history):
    ...
    for entry in gradio_history:
        ...
        if isinstance(content, gr.Image):
            continue
        if isinstance(content, gr.Video):
            continue
...
def main(preset: str = "qwen"):
    ...
    tools = [search, current_time, generate_image, generate_video]
